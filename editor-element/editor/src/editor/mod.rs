...
use utils::*;
pub use utils::{get_ast_time, element};
...
pub fn Editor<'a>(
    ...
    let code_element = move || -> HtmlTextAreaElement { element(&code_id(), &container_ref) };
    #[allow(unused)]
    let code_outer_element = move || -> HtmlDivElement { element(&code_outer_id(), &container_ref) };
    let glyph_doc_element = move || -> HtmlDivElement { element(&glyph_doc_id(), &container_ref) };
    ...
    let get_code_cursor = move || get_code_cursor(&code_id(), &container_ref);
    ...
    let get_code = move || get_code(&code_id(), &container_ref);
    ...
    window_event_listener(mousemove, move |event| {
        if let Some(overlay_element) = get_element::<HtmlDivElement>(&overlay_id(), &container_ref) {
    ...
    let code_mouse_move = move |event: MouseEvent| {
        ...
        let overlay: HtmlDivElement = element(&overlay_id(), &container_ref);
        ...
        let hover_elem: HtmlDivElement = element(&hover_id(), &container_ref);
    ...
    view! {
        ...
        <div id="editor-wrapper" _ref=container_ref>
            <div id="editor">
