...
use crate::{
    editor::backend::{OutputItem, WebBackend},
    editor::{Editor, code_font, prim_sig_class, binding_class},
};
...
impl State {
    ...
    pub fn set_code(&mut self, code: &str, cursor: Cursor) {
        ...
        let maybe_before = get_code_cursor(&self.code_id, &self.container_ref);
    ...
    pub fn track_change(&mut self) {
        let Some(cursor) = get_code_cursor(&self.code_id, &self.container_ref) else {
            return;
        };
        let code = get_code(&self.code_id, &self.container_ref);
    ...
    fn set_code_element(&self, code: &str) {
        ...
        if let Some(cursor) = get_code_cursor(&self.code_id, &self.container_ref) {
            self.set_cursor(cursor);
        }
        self.set_overlay.set(code.into());
        let area = element::<HtmlTextAreaElement>(&self.code_id, &self.container_ref);
        let outer = element::<HtmlDivElement>(&self.code_outer_id, &self.container_ref);
    ...
    fn set_line_count(&self) {
        self.set_line_count
            .set(get_code(&self.code_id, &self.container_ref).split('\n').count().max(1));
    }
...
pub fn get_code(id: &str, container_ref: &NodeRef<Div>) -> String {
    element::<HtmlTextAreaElement>(id, &container_ref).value()
}
...
pub fn get_code_cursor(id: &str, container_ref: &NodeRef<Div>) -> Option<(u32, u32)> {
    let area = element::<HtmlTextAreaElement>(id, &container_ref);
    let content = area.value();
...
pub fn get_element<T: JsCast>(id: &str, container_ref: &NodeRef<Div>) -> Option<T> {
    let mut id_selector = "#".to_owned();
    id_selector.push_str(id);

    let container = container_ref.get_untracked().expect("Container loaded");
    if let Some(elem) = container.query_selector(id_selector.as_str()).unwrap() {
...
#[track_caller]
pub fn element<T: JsCast>(id: &str, container_ref: &NodeRef<Div>) -> T {
    if let Some(elem) = get_element(id, container_ref) {
        elem
    } else {
        panic!("#{id} not found")
    }
